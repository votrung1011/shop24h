(function ($, window, document, undefined) {
   ...
   $.extend(WeekPicker.prototype, {
      init: function () {
         ...
         $("body")
            .on("mousemove", ".ui-weekpicker .ui-datepicker-calendar tr", function () {
               $(this).find("td a").addClass("ui-state-hover");
            })
            .on("mouseleave", ".ui-weekpicker .ui-datepicker-calendar tr", function () {
   ...
   $.fn.weekpicker = function (options) {
      return this.each(function () {
         if (!$.data(this, "plugin_" + pluginName)) {
            $.data(this, "plugin_" + pluginName, new WeekPicker(this, options));
         }
      });
   };
   ...
   var createWeekPickerInput = function (datePickerInput) {
      ...
      var weekPickerInput = $(
         '<input type="text" id="' + weekPickerId + '" data-datepicker-id="' + datePickerId + '">'
      );
   ...
   var leftPad = function (input, length, padString) {
      ...
      input = input + "";
      return input.length >= length ? input : new Array(length - input.length + 1).join(padString) + input;
   };
